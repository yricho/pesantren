...
export default function PPDBAdminPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const [registrationsRes, statsRes] = await Promise.all([
        fetch(
          `/api/ppdb/admin/registrations?page=${page}&limit=20&status=${statusFilter}&level=${levelFilter}&paymentStatus=${paymentFilter}`
        ),
        fetch("/api/ppdb/admin/stats"),
      ]);

      const registrationsData: RegistrationResponse =
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/ppdb/admin/registrations?search=${encodeURIComponent(
          searchTerm
        )}&page=1&limit=20`
      );
  ...
  const handleExport = async () => {
    try {
      ...
      if (response.ok) {
        ...
        a.href = url;
        a.download = `ppdb-registrations-${format(
          new Date(),
          "yyyy-MM-dd"
        )}.xlsx`;
  ...
  const handleQuickAction = async (
    id: string,
    action: "verify" | "reject" | "schedule-test"
  ) => {
    try {
      let endpoint = "";
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            Anda tidak memiliki akses ke halaman admin PPDB.
          </p>
          <Link
            href="/dashboard"
                    ...
                    <div className="flex space-x-2">
                      ...
                      {registration.status === "SUBMITTED" && (
                        <>
                          <button
                            onClick={() =>
                              handleQuickAction(registration.id, "verify")
                            }
                          ...
                          <button
                            onClick={() =>
                              handleQuickAction(registration.id, "reject")
                            }
                      ...
                      {registration.status === "VERIFIED" && (
                        <button
                          onClick={() =>
                            handleQuickAction(registration.id, "schedule-test")
                          }
